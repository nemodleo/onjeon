  ...
  return (
    <>
      ...
      <div className="space-y-4">
        {/* Header */}
        <div className="pt-2">
          <h1 className="text-xl font-bold text-black mb-1">환전 내역</h1>
          <p className="text-gray-600 text-xs">거래 이력 • 수수료 절약액 확인</p>
        </div>
        ...
        <div className="bg-black rounded-xl p-4 text-white">
          <div className="flex justify-between items-start mb-2">
            <div>
              <p className="text-gray-300 text-xs">이번 달 환전액</p>
              <div className="flex items-baseline space-x-2">
                <p className="text-lg font-bold">{formatCurrency(totalVolume, 'KRW')}</p>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-gray-50 rounded-xl p-3">
            <div className="text-lg font-bold text-black">{transactions.filter(t => t.status === 'completed').length}</div>
            <div className="text-xs text-gray-600">완료</div>
          </div>
          <div className="bg-gray-50 rounded-xl p-3">
            <div className="text-lg font-bold text-orange-500">{transactions.filter(t => t.status === 'processing').length}</div>
            <div className="text-xs text-gray-600">처리중</div>
          </div>
          <div className="bg-gray-50 rounded-xl p-3">
            <div className="text-lg font-bold text-green-500">{formatCurrency(totalSaved, 'KRW')}</div>
        ...
        <div className="space-y-2">
          <h3 className="text-base font-bold text-black">기간별 조회</h3>
            ...
            {transactions.slice(0, 5).map((transaction) => (
              <div key={transaction.id} className="flex items-center justify-between p-3 bg-white rounded-xl border border-gray-100">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-green-600">+{formatCurrency(transaction.savedFee, 'KRW')} 절약</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Savings Summary */}
        <div className="space-y-3">
          <h3 className="text-base font-bold text-black">수수료 절약 분석</h3>
          <div className="space-y-1">
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-xl border border-green-100">
              <div>
                <div className="text-sm font-semibold text-black">이번 달 절약</div>
            ...
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl border border-blue-100">
              <div>
                <div className="text-sm font-semibold text-black">연간 예상 절약</div>
